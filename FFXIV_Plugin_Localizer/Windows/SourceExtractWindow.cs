...
public sealed class SourceExtractWindow : Window
{
    ...
    public override void Draw()
    {
        ...
        ImGui.TextUnformatted("访问 GitHub 的代理设置：");
        ...
        if (ImGui.IsItemHovered())
            ImGui.SetTooltip("只有勾选此项、并填写代理地址后，本功能才会访问 GitHub。\n未勾选时即使填了地址也不会联网。");

        ImGui.SameLine();
        ImGui.SetNextItemWidth(Math.Max(200f, ImGui.GetContentRegionAvail().X - 60f));
        var proxy = cfg.ProxyAddress;
        if (ImGui.InputTextWithHint("##ProxyAddr", "代理地址，如 http://127.0.0.1:7890 或 socks5://127.0.0.1:1080", ref proxy, 256))
        {
            cfg.ProxyAddress = proxy.Trim();
            cfg.Save();
        }
        if (ImGui.IsItemHovered())
            ImGui.SetTooltip("本机代理地址（Clash/v2ray 等）。只保存在本机配置文件，不会随插件分发。");

        // 状态灯：条件是否满足一目了然
        if (cfg.CanAccessGitHub)
            Ui.ColoredWrapped(new Vector4(0.55f, 0.9f, 0.55f, 1f), "✔ 已启用代理并填写地址：可以访问 GitHub 拉取源码。");
        else if (!cfg.UseProxy)
            Ui.ColoredWrapped(new Vector4(1f, 0.6f, 0.35f, 1f), "✘ 未勾选「启用代理」：不能访问 GitHub（即使填了地址也不会联网）。");
        else
            Ui.ColoredWrapped(new Vector4(1f, 0.6f, 0.35f, 1f), "✘ 已勾选启用代理，但代理地址为空：请填写后再试。");
